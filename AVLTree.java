...
public class AVLTree<E extends Comparable<E>> extends BST<E> {
	...
	/*
	 * this is a tail recursive method that will find the kth smallest element in an AVLTree.
	 */
	public E find(int k){
		AVLTreeNode<E> root = (AVLTreeNode<E>)this.root;
		return find(k, root);
	}
	...
	/** Also updates the size of the AVLTReeNode. */
	private void updateHeight(AVLTreeNode<E> node) {
		if (node.left == null && node.right == null){ // node is a leaf
			...
			node.size = 1 +     A.getSize() + B.getSize();
		}
	}
